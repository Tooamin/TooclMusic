<template>
  <div class="song" ref="song">
      ...
      <div class="big-player" v-show="this.isPlaystatus">
        ...
        <div class="player-lyrics">
          ...
          <div class="lyrics-box" v-show="islyrics" @click="lyricsModel">
            <div class="lyricitp" v-if="lyric.length == 0">暂无歌词</div>
            <van-row type="flex" justify="center">
              <van-col span="23" class="lyrics-move" ref="lyricmove">
                <van-row
                  ...
                  :style="{
                    'font-size': index == currentrow ? '18px' : '',
                    color: index == currentrow ? '#2ddae8' : '',
                  }"
                >
                  {{ item.text }}
                </van-row>
    ...
    <div class="playerswitch-false" @click="minPlayerswitch">
      <img
        class="auto-img"
        src="../assets/images/playerswitch.png"
        alt=""
        srcset=""
      />
    </div>
    <div class="playerswitch-true" @click="minPlayerswitch">
      <img src="../assets/images/loseminplayers.png" alt="" class="auto-img" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateTime() {
      ...
      this.notcurrentTime = Math.round(currentTime);
    ...
    // 小播放器开关
    minPlayerswitch() {
      if (this.minplayerstatus == false) {
        this.$refs.song.style.right = 0 + "px";
        this.minplayerstatus = true;
      } else if (this.minplayerstatus == true) {
        this.$refs.song.style.right = -376 + "px";
        this.minplayerstatus = false;
      }
    },
  },
  computed: {
