<template>
  <div class="search">
    ...
    <div class="secach-box">
      <van-search v-model="userinputvalue" placeholder="请输入搜索关键词" @input="getSearchData" @search='getSearchData' />
    </div>
      ...
      <div class="history-search" v-if="historylistData.length > 0">
        <div class="history-title">
          历史搜索<span class="clear-icon"
             @click="clearHistory"><img src="../assets/images/clear.png" alt="" class="auto-img"
          /></span>
        </div>
        <div class="history-contentlist">
          <div class="history-item" v-for="(item,index) in historylistData" :key="index">{{historylistData[index]}}</div>
        </div>
      </div>
      <div class="hotsearch">
    ...
    <div class="resultslist" v-if="userinputvalue.length">
      <div class="result-item van-ellipsis" v-for="(item,index) in searchData" :key="index" @click="clickMusicGetid(item.id)"><van-icon name="search" />{{item.name}}-{{item.ar[0].name}}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    clickMusicGetid(id){
        ...
        localStorage.setItem("historySearch",this.historylistData);
        this.userinputvalue = '';
