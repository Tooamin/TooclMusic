  ...
  <div class="my">
    ...
    <div class="userinfo-box">
      <div class="userinfo" @click="popupdetail">
        <div class="user-img"><img :src="userinfoData.avatarUrl+'?param=100y100'" alt="" /></div>
        <div class="usertitle">
      ...
      <div class="popup">
        <van-popup v-model="ispopup"  position="bottom" :style="{ height: '18%' }">
          <van-button type="danger" @click="outLogin">退 出</van-button>
        </van-popup>
      </div>
      <div
        class="likemusic"
      ...
      >
        <div class="likemusic-img">
          <img :src="likemusicData.coverImgUrl+'?param=100y100'" alt="" class="auto-img" />
        </div>
...
export default {
  ...
  methods: {
    ...
    outLogin(){
      ...
      localStorage.removeItem("__icoo");
      ...
      this.axios({
        ...
        .then((result) => {
          ...
          for (let i = 1; i < result.data.playlist.length; i++) {
            ...
            } else if(result.data.playlist[i].creator.nickname !=
              this.userinfoData.nickname){
              collection.push(result.data.playlist[i]);
            }
          }
          this.createplaylistData = create;
          this.collectionplaylistData = collection;
        })
        .catch((err) => {
          this.$toast.clear();
          this.$toast("获取失败");
