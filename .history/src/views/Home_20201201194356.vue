    ...
    <div class="anewsong">
      <Newmusicblock :left-title="'新歌推荐'">
        <div class="anewsong-box">
          <div class="anewsong-item" v-for="(item,index) in newSongData" :key="index">
            <div class="anewsong-img auto-img">
              <img :src="item.album.picUrl" alt="">
            </div>
            <div class="anewsong-name">
              <div class="anewsong-title van-ellipsis">{{item.album.name}}</div>
              <div class="anewsong-subtitle van-ellipsis" v-for="(v,i) in item.artists">{{item.artists[0].name}}</div>
            </div>
    ...
    getNewsongData(){
      ...
      this.axios({
        ...
        for(let i = 0 ; i < 8;i++){
         ...
         (this.newSongData[0].artists[0],this.newSongData[0].artists[1]);
         (this.newSongData);
      }).catch((err) =>{
        this.$toast.clear();
        this.$toast("加载失败")
