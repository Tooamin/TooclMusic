...
export default {
  ...
  methods: {
    ...
    clickMusicGetid(id) {
      ...
      for (let i = 0; i < this.historylistData.length; i++) {
        if (listData.indexOf(this.historylistData[i]) === -1) {
          listData.push(this.historylistData[i]);
        }
      }
      this.historylistData = listData;
      let historylist = JSON.stringify(listData);
      localStorage.setItem("historySearch", listData);
      this.userinputvalue = "";
