...
export default {
  ...
  methods: {
    ...
    updateCurrent(){
      this.currentBar == 100 ? (this.clearTimer(),this.loopStyle == "not" ? this.notLoop() : this.loopStyle == "list" ? this.listLoop() : this.loopStyle == "single" ? this.singleLoop() : this.randomLoop()) : (this.currentBar = (this.$refs.audio.currentTime / this.$refs.audio.duration) * 100),
      this.$store.commit("getSongtime",this.$refs.audio.currentTime);
    }
  },
  computed: {
    //当前播放信息
