<template>
  <div class="my">
    ...
    <div class="userinfo-box">
      ...
      <div class="likemusic" @click="gojump('SonglistDateil',likemusicData.id)">
      ...
      <div class="createplaylist">
          ...
          <div class="ceateplaylist-box">
              <Likemusicitem v-for="(item,index) in createplaylistData" :key="index" :like-img="item.coverImgUrl" :like-title="item.name" :like-number="item.trackCount" @go-jump="gojump('SonglistDateil',item.id)" ></Likemusicitem>
          </div>
      </div>
      <div class="my-collection">
        <div class="createplaylist-title">收藏的歌单</div>
          <div class="ceateplaylist-box">
              <Likemusicitem v-for="(item,index) in collectionplaylistData" :key="index" :like-img="item.coverImgUrl" :like-title="item.name" :like-number="item.trackCount"  @go-jump="gojump('SonglistDateil',item.id)" ></Likemusicitem>
          </div>
      </div>
    </div>
  </div>
      ...
      this.axios({
        ...
        .then((result) => {
          ...
          for(let i = 1 ; i < result.data.playlist.length;i++){
              if(result.data.playlist[i].creator.nickname == this.userinfoData.nickname){
