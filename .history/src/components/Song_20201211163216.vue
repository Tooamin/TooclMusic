  ...
  <div class="song">
    <van-sticky>
      <div class="big-player" v-show="this.isPlaystatus">
        <div
          class="songbg"
          :style="{ backgroundImage: 'url(' + songUrl+'?param=100y100' + ')' }"
        ></div>
          ...
          <van-nav-bar left-arrow :border="false" @click-left="goBack">
            <template #title>
              <div class="title-box">
                ...
                <div class="singer-name" @click="toggleSongerDetail" v-if="singerName">
...
export default {
  ...
  methods: {
    ...
    formatLyric(value) {
      ...
      this.$store.commit("getLyrics", this.lyricsData); //把歌词提交到store里，为了重新进入该组件时还能再次渲染
    },
    //排序
