<template>
  <div class="song">
    ...
    <div class="big-player" v-if="this.isPlaystatus">
      ...
      <div class="min-player" @click="changeStateOfPlayer" v-if="!this.isPlaystatus">
        <div class="min-play-box">
          <div class="min-play-img">
            <img :src="songUrl" alt="" class="auto-img" />
          </div>
          <div class="min-play-name">
            <div class="min-play-title">{{musicDate.name}} - {{singerName}}</div>
...
export default {
  ...
  methods: {
    ...
    updateCurrent() {
      ...
      this.currentBar == 100
        ? (this.clearTimer(),this.loopStyle == "list" ? this.listLoop() : this.loopStyle = "random" ? this.randomLoop() : this.loopStyle == )
        : (this.currentBar =
