    ...
    <div class="big-player" v-show="this.isPlaystatus">
        ...
        <div class="player-box">
          <van-nav-bar left-arrow :border="false" @click-left="goBack">
            <template #title>
              <div class="title-box">
                <div class="player-name">{{ musicDate.name }}<span v-if="musicDate.name == 0">电台</span></div>
                <div class="singer-name" @click="toggleSongerDetail">
        ...
        <div class="song-imgbox" v-show="!islyrics" @click="lyricsModel">
          <div class="songimg">
        ...
        <div class="popup">
          ...
          >
            <div class="popup-content">
              ...
              >
                <div class="song-name">
                  <div class="song-name-title van-ellipsis">{{ item.name }} - {{listSingerName[index]}}</div>
      ...
      <div class="min-player"  v-if="!this.isPlaystatus">
        <div class="min-play-box" @click="changeStateOfPlayer">
            ...
            <div class="min-play-name">
              <div class="min-play-title van-ellipsis"><span v-if="musicurl == true">{{musicDate.name}} - {{singerName}}</span><span v-if="musicDate.name == 0">电台</span><span v-if="musicurl == false">当前为播放歌曲</span></div>
              <div class="min-play-time"><div class="starttime">{{currentText}}</div><div class="endtime">{{durationText}}</div></div>
              <div class="min-play-range"><van-slider v-model="currentBar" @input="dragStart" /></div>
            </div>
          </div>
          <div class="playsong">
              <img
...
export default {
  ...
  methods: {
    //歌手详情
    toggleSongerDetail(){
      this.$router.push({name:"SingerDetail", query:{singerid: this.musicDate.ar[0].id}})
    },
      ...
      this.axios({
        ...
        .then((result) => {
          ...
          if (result.data.code == 200) {
            ...
            this.listSingerName.indexOf(this.singerName) == -1 ? this.listSingerName.push(this.singerName) : '';
    ...
    updateCurrent() {
      ...
      this.currentBar == 100
        ? (this.clearTimer(),this.loopStyle == "list" ? this.listLoop() : this.loopStyle = "random" ? this.randomLoop() : this.notLoop())
        : (this.currentBar =
