    ...
    <div class="rankingshow-box">
      ...
      >
        <div class="ranking-img">
          <img v-lazy="item.picUrl" alt="" class="auto-img" />
        </div>
...
export default {
  ...
  methods: {
    ...
    //获取id
    getSingerId(singerid) {
      this.$router.push({ name: "SingerData", singerid: { singerid } });
    },
    getSingerinfo() {
      this.$toast.loading({
        message: "加载中...",
      ...
      this.axios({
        ...
        .then((result) => {
          this.$toast.clear();
          this.singerInfo = result.data.list.artists;
        })
        .catch((err) => {
          this.$toast.clear();
          this.$toast("加载失败");
