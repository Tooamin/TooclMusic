        ...
        <div class="player-lyrics">
          ...
          <div class="lyrics-box" v-show="islyrics" @click="lyricsModel">
            <van-row type="flex" justify="center">
              <van-col span="23" :style="lyricsmove" class="lyrics-move" ref="lyricmove">
                <van-row
...
export default {
  ...
  watch: {
    ...
    lyricCurrent() {
      ...
      this.lyricsmove = this.$refs.lyricmove;
         
      this.lyricsData.forEach((element, index) => {
        if (this.lyricCurrent == element.time) {
          this.lyricsmove.top = -index * 2.5 + 6 + "rem";
    ...
    formatLyric(value) {
      ...
      for (let i = 0; i < row; i++) {
        ...
        temp_arr.forEach((element) => {
          ...
          obj.time = s;
          obj.text = text;
          this.lyricsData.push(obj);
        });
      }
      this.lyricsData.sort(this.sort);
      this.$store.commit("getLyrics", this.lyricsData);
    },
    //排序
    sort(a, b) {
  ...
  computed: {
    ...
    //播放时间
    songcurrent() {
      return this.$store.state.songcurrent;
    },
    lyricslistData(){
      return this.$store.state.lyricsData;
    }
  },
};
</script>
