        ...
        <div class="player-lyrics">
          ...
          <div class="lyrics-box" v-show="islyrics" @click="lyricsModel">
            <van-row type="flex" justify="center">
              <van-col span="100" :style="lyricsmove">
                <van-row v-for="(item, index) in lyricsData" :key="index" class="lyrics-centent">
                  {{ item.text }}
                </van-row>
              </van-col>
            </van-row>
  ...
  watch: {
    ...
    lyricsCurrenttime(){
      ...
      this.lyricsData.forEach((element,index) =>{
        if(this.lyricsCurrent == element.tiem){
          this.lyricsmove.top = -index * 2.5 + 6 + "rem";
          this.lyricsCurrent = index;
        }
      })
    }
  },
