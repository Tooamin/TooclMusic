    ...
    <div class="big-player" v-show="this.isPlaystatus">
        ...
        <div class="player-box">
          <van-nav-bar left-arrow :border="false" @click-left="goBack">
            <template #title>
              <div class="title-box">
                <div class="player-name">{{ musicData.name }}<span v-if="musicData.name == 0">电台</span></div>
      ...
      <div class="min-player"  v-if="!this.isPlaystatus" v-show="isPlayShow">
        <div class="min-play-box" @click="changeStateOfPlayer">
            ...
            <div class="min-play-name">
              <div class="min-play-title van-ellipsis">{{musicData.name}} - {{singerName}}</div>
...
export default {
  ...
  methods: {
    //歌手详情
    toggleSongerDetail(){
      this.$router.push({name:"SingerDetail", query:{singerid: this.musicData.ar[0].id}})
    },
      ...
      this.axios({
        ...
        .then((result) => {
          ...
          if (result.data.code == 200) {
            ...
            this.musicData = result.data.songs[0];
               
            let str = "";
            for(let i = 0; i < this.musicData.ar.length;i++){
                str += this.musicData.ar[i].name+ "-";
            }
